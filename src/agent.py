...
class PeterGriffinAgent:
        ...
        try:
            ...
            if posts:
                ...
                if len(interesting_posts) < num_comments:
                    interesting_posts = posts
                
                for i in range(num_comments):
                    if i < len(interesting_posts):
            ...
            if posts and random.random() < 0.8:
                num_upvotes = min(random.randint(3, 6), len(posts))
            ...
            logger.info(f"Completed {actions_done} actions this cycle")
            self.error_count = 0
    ...
    def _comment_on_single_post(self, post: dict):
    ...
    def _upvote_single_post(self, post: dict):
